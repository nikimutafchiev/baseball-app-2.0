    ...
    return (
        <div className="w-full flex flex-row justify-around px-8 py-2 rounded items-center text-gray-500 bg-white  font-semibold  drop-shadow-xl">
            ...
            <div className="flex flex-row justify-around items-center w-1/2">
                <div className={`w-2/5 ${props.size == "normal" ? "text-xs" : "text-3xs"} flex flex-row gap-4 items-center`}><img src={props.home.logo}></img><div className='w-1/2 text-center'>{props.home.teamName}</div></div>
                <div className={`${props.size == "normal" ? "text-xl" : "text-lg"} font-bold w-1/5`}>{props.home.result} - {props.away.result}</div>
                <div className={`w-2/5 ${props.size == "normal" ? "text-xs" : "text-3xs"} flex flex-row gap-4 items-center`}><div className='w-1/2 text-center'>{props.away.teamName}</div><img src={props.away.logo}></img></div>
            </div>
