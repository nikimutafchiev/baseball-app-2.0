...
export default function Game(props) {
    ...
    return (
        <div className="w-full flex flex-row justify-around px-8 py-2 rounded items-center text-gray-500 bg-white  font-semibold  drop-shadow-xl">
            ...
            <button className={`w-[10%] px-1 py-2 bg-orange-400 rounded text-white ${props.size == "normal" ? "text-xs" : "text-3xs"} hover:bg-orange-300 flex flex-row items-center gap-1 justify-center ${props.isAdmin ? "invisible" : ""}`}><div>Score game</div><RiArrowRightCircleLine size={15} /></button>
        </div>
    )
}