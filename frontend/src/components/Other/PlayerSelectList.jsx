...
export default function PlayerSelectList(props) {
    ...
    return (<><div className="flex flex-col top-6 bg-white h-4/5 overflow-y-auto py-2 px-4 fixed self-center z-20 rounded w-2/3">
        <div className="flex flex-row justify-between items-start">
            <TextField value={searchInput} onChange={(e) => setSearchInput(e.target.value)} label={<div className="flex flex-row gap-1 items-center">{searchIcon}<div>Search</div></div>} className="bg-white w-1/4 rounded" />
