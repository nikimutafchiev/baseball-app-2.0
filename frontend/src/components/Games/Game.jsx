...
export default function Game(props) {
    ...
    return (
        <div className="w-full grid md:grid-cols-9 grid-cols-1 gap-2 justify-around min-h-[60px] px-8 py-2 rounded place-items-center text-gray-500 bg-white  font-semibold  drop-shadow-xl">
            ...
            <div className="grid grid-cols-5 justify-around items-center md:col-span-5 w-full">
            ...
            <Link className={`w-2/5 md:w-full ${props.size == "normal" ? "text-xs" : "text-2xs"} py-2 rounded bg-blue-500 hover:bg-blue-400 text-white  ease-in-out text-center duration-150`} to={`/games/${props.id}`}>More Info</Link>
            <Link to={`/score/${props.id}`} className={`w-2/5 md:w-full px-1 py-2 bg-orange-400 rounded text-white text-nowrap ${props.size == "normal" ? "text-xs" : "text-2xs"} hover:bg-orange-300 flex flex-row items-center gap-1 justify-center ${props.isAdmin ? "invisible" : ""}`}><div>Score game</div><RiArrowRightCircleLine size={15} /></Link>
        </div>
    )
}