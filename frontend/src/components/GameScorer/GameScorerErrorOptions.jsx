    ...
    return (<div className="fixed inset-0 z-10 bg-black bg-opacity-50">
        <div className="fixed z-20 inset-0 flex flex-col gap-4 text-white font-semibold text-4xl bg-white w-1/2 h-[90%] self-center justify-self-center rounded">
            ...
            <div className="grid grid-cols-10 gap-y-4 px-6">
                ...
                <div className="col-span-2"></div>
                <button className="mt-4 bg-blue-500 hover:bg-blue-400 rounded text-center  p-2 content-center col-span-2 text-lg" onClick={() => { if (!props.isRunner) props.situationFunction(positions.length == 2 ? `${positions[0]}E${positions[1]}` : positions.length == 1 ? (errorType === "Throwing" ? `E${positions[0]}T` : `E${positions[0]}`) : "", positions.length == 2 ? [positions[0]] : [], positions.length == 2 ? [positions[1]] : [positions[0]]); else props.runnerSituationFunction(positions.length == 2 ? `${positions[0]}E${positions[1]}` : positions.length == 1 ? (errorType === "Throwing" ? `E${positions[0]}T` : `E${positions[0]}`) : "", positions.length == 2 ? [positions[0]] : [], positions.length == 2 ? [positions[1]] : [positions[0]]) }}>Confirm</button>
            </div>
