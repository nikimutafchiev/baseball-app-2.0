...
export default function GameScorerRunnerOptions(props) {
    ...
    const runnerOptionsComponents = {
        "Error": <GameScorerErrorOptions runnerSituationFunction={(errorSituation) => { props.situationFunction(props.runner.player, errorSituation, props.runner.oldBasePosition, basePosition); setSituationOption(""); props.close() }}
            isRunner={true} />,
        "Force out": <GameScorerOutOptions runnerSituationFunction={(positions) => { props.situationFunction(props.runner.player, `Force out ${positions}`, props.runner.oldBasePosition, null, true); setSituationOption(""); props.incrementOuts(); props.close() }} isRunner={true} />,
        "Tagged out": <GameScorerOutOptions runnerSituationFunction={(positions) => { props.situationFunction(props.runner.player, `Tagged out ${positions}`, props.runner.oldBasePosition, null, true); setSituationOption(""); props.incrementOuts(); props.close() }} isRunner={true} />,
        "Caught stealing": <GameScorerOutOptions runnerSituationFunction={(positions) => { props.situationFunction(props.runner.player, `Caught stealing ${positions}`, props.runner.oldBasePosition, null, true); setSituationOption(""); props.incrementOuts(); props.close() }} isRunner={true} />,
        ...
        <div className="fixed inset-0 z-10 bg-black bg-opacity-50">
            <div className="fixed z-20 flex flex-col  inset-0  py-2 overflow-y-hidden text-white font-semibold bg-white w-3/4 h-4/5 self-center justify-self-center rounded">
                ...
                <div className="flex-1 grid grid-cols-3 overflow-y-auto gap-y-2 gap-x-4 text-white text-2xl font-semibold py-4 px-4 z-10">
                    ...
                    <div className="bg-primary_2 hover:bg-primary_2_hover p-2 px-4  flex flex-row justify-between items-center cursor-pointer transform transition-transform hover:scale-105 rounded-lg" onClick={() => { props.situationFunction(props.runner.player, "SB", props.runner.oldBasePosition, basePosition); props.close() }}><div>SB</div><div>Stolen base</div></div>
